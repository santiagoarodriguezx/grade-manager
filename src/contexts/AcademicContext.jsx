...
export const AcademicProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (academicSelection) {
      localStorage.setItem('academicSelection', JSON.stringify(academicSelection))
    } else {
  ...
  const hasAcademicSelection = () => {
    return academicSelection !== null && academicSelection.module && academicSelection.trimester
  }
